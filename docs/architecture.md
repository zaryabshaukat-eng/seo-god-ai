...
- `plugin-sdk` has **zero runtime dependencies** and owns the only sandbox in the platform: third-party plugin code is executed inside a closed V8 `vm` context with a hard timeout, cloned inputs/outputs, and an explicit permission model. The `api` application consumes the SDK's `PluginRegistry` structurally through `apps/api/src/platform.ts`.
