...
describe('ids', () => {
  it('deterministicUuid is stable per input and distinct across inputs', () => {
    const a = deterministicUuid('store-1|product|123|update_title');
    const b = deterministicUuid('store-1|product|123|update_title');
    const c = deterministicUuid('store-1|product|123|update_url');
    expect(a).toBe(b);
    expect(a).not.toBe(c);
